...
DEFAULT_ROOT = 'smetrics'


def _check_cython_version():
    message = ('Please install Cython with a version >= {0} in order '
               'to build a smetrics from source.').format(
                    CYTHON_MIN_VERSION)
    try:
        import Cython
...
def cythonize_extensions(top_path, config):
    ...
    # check simple compilation with OpenMP. If it fails smetrics will
    ...
    # smetrics module (only at build-time) to be used twice:
    # - First to set the value of smetrics_OPENMP_PARALLELISM_ENABLED, the
    ...
    smetrics._OPENMP_SUPPORTED = check_openmp_support()
    ...
    config.ext_modules = cythonize(
        ...
        nthreads=n_jobs,
        compile_time_env={
            'smetrics_OPENMP_PARALLELISM_ENABLED': smetrics._OPENMP_SUPPORTED},
